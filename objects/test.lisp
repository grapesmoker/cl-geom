...
#|
(lisp-unit2:define-test test-make-vec3
  (let ((new-vec (make-vec3 1 2 3)))
    (loop
       for i below 3
       do
	 (lisp-unit2:assert-equal (+ i 1) (v-ref new-vec i)))))
  
(lisp-unit2:define-test test-vector-addition
  (let* ((v1 (make-vec3 1 2 3))
	 (v2 (make-vec3 4 5 6))
	 (result (v+ v1 v2)))
    (lisp-unit2:assert-true (and (equalp (vector-contents result) '(5 7 9))))))
|#
